...
(function () {
    ...
    var te = window.textEditor = {
        ...
        configureEvents: function() {
            ...
            // Ctrl-Alt key handling that's forwarded to WPF
            keySpecialNeedsToSend = null;
            keySpecialDownTime = null;

            //$("pre[lang]").on("keydown",
                //function(event) {
                //    // check for alt and ctrl keys and any keys pressed 2 seconds after
                //    if (event.altKey || event.ctrlKey) {
                //        keySpecialDownTime = new Date();
                //        keySpecialNeedsToSend = event.altKey ? "alt" : "ctrl";
                //        console.log('alternate key: ' + event.altKey + " -  " + event.ctrlKey + " - " + keySpecialNeedsToSend);
                //    }
                //});
            ...
            $("pre[lang]").on("keyup",
                function (event) {
                    ...
                    //if (event.altKey || event.ctrlKey) {
                    //    keySpecialDownTime = new Date();
                    //    console.log('alternate key: ' + event.altKey + event.ctrlKey);
                    //    chrome.webview.hostObjects.mm.ForwardSpecialKeyCombos(-1, event.altKey, event.ctrlKey);
                    //    event.preventDefault();
                    //    return;
                    //}

                    //try {
                    //    if (keySpecialDownTime && new Date() - keySpecialDownTime < 2200) {
                    //        if (keySpecialNeedsToSend) {
                    //            var ctrl = keySpecialNeedsToSend === "ctrl";
                    //            var alt = keySpecialNeedsToSend === "alt";
                    //            chrome.webview.hostObjects.mm.ForwardSpecialKeyCombos(-1, alt, ctrl );
                    //            //chrome.webview.hostObjects.mm.Helloworld("Rick Wreckless");
                    //            console.log("sending alternate key");
                    //        }
                    //        keySpecialNeedsToSend = null;
                    //        //chrome.webview.hostObjects.mm.ForwardSpecialKeyCombos(event.keyCode, false, false);
                    //        event.preventDefault();
                    //        return false;
                    //    }
                    //    keySpecialNeedsToSend = null;
